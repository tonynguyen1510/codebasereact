...
import { updateTestingDetail } from 'src/redux/actions/testingDetail';
...
export default class BtnAddComment extends PureComponent {
	static propTypes = {
		testingDetailData: PropTypes.object.isRequired,
	...
	handleOk = () => {
		if (this.state.comment && this.props.testingDetailData.id) {
			this.setState({
				confirmLoading: true,
			});

			this.props.action.updateTestingDetail({ id: this.props.testingDetailData.id, updatedAt: new Date(), comment: this.state.comment }, () => {
